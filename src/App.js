...
function App() {
  ...
  const submit = (type_code) => type_code === 'FILE' ? fileSubmit() : serverSubmit();

  const fileSubmit = () => dispatch(formSubmit({
    'resource_type': "FILE",
  ...
  const serverSubmit = () => dispatch(formSubmit({
  ...
  useEffect(() => {
    ...
    setInterval(() => {
      ...
      if ( date.getHours() < 8 || date.getHours() === 12 || date.getHours() > 16 ) dispatch(setActivity('NOWORK'));
      else if ( Math.round(Math.random()) % 2 ) {
          dispatch(getRemote());
          if ( Math.round(Math.random()) % 2 ) {
            dispatch(getResourceList()); 
            let action = Math.round(Math.random()) % 2 
            let resurce = Math.round(Math.random()) % 2
            if ( resurce ) if ( action ) dispatch(getFilePlaceList()); else dispatch(getFileResourcesList());
            else if ( action ) {
          ...
          } else dispatch(setActivity(`${time} REMOTE`)); 
        }
    }, interval)
